...
import logging
import io
from ..services import user_service, set_service, stats_service
...
# --- BẮT ĐẦU THÊM MỚI: Route cho Admin Dashboard ---
@admin_bp.route('/')
@admin_bp.route('/dashboard')
@admin_required
def dashboard():
    """
    Mô tả: Hiển thị trang dashboard chính cho quản trị viên.
    """
    admin_stats = stats_service.get_admin_dashboard_stats()
    if not admin_stats:
        flash("Không thể tải dữ liệu cho Admin Dashboard.", "error")
        admin_stats = {} # Tránh lỗi trong template
...
# --- BẮT ĐẦU THÊM MỚI: Route xuất Excel ---
@admin_bp.route('/sets/export/<int:set_id>')
@admin_required
def export_set(set_id):
    """
    Mô tả: Xử lý yêu cầu xuất một bộ thẻ ra file Excel.
    """
